...
class LoanValidator {
  ...
  static validateLoanApplication(req, res, next) {
    ...
    if (isNaN(amount) || amount < 1000 || amount > 10000000) {
      errorMessage.amount =
        'amount should be number & not less than 1000 or above 10000000';
    }
    if (isNaN(tenor) || tenor < 1 || tenor > 12) {
      errorMessage.tenor =
        'tenor should be number & not less than 1 or above 12';
    }
  ...
  static validateQueryParams(req, res, next) {
    ...
    if (!(loanRepaid == 'true' || loanRepaid == 'false')) {
      errorMessage.repaid =
        'repaid value is required & should be true or false';
    }
    if (
      !(
        loanStatus == 'approved' ||
      ...
      errorMessage.status =
        'status value is required & should be pending, approved, rejected';
    }
    if (!(Object.keys(errorMessage).length === 0)) {
      return res.status(400).json({ status: 'failed', error: errorMessage });
    }
