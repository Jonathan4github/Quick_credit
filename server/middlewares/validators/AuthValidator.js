...
class UserValidation {
  static Signup(req, res, next) {
    ...
    db.query(`SELECT id FROM users WHERE email = '${email}'`).then(
      userfound => {
        ...
        if (!validator.isLength(firstName, { min: 2, max: 20 })) {
          errorMessage.firstName =
            'First name must not be less than 2 or above 17 characters';
        }
        if (!validator.isLength(lastName, { min: 2, max: 20 })) {
          errorMessage.lastName =
            'Lastname must not be less than 2 or above 17 characters';
        }
        ...
        if (!validator.isLength(password, { min: 7, max: 20 })) {
          errorMessage.password =
            'password must not be less than 7 or above 10 characters';
        }
        if (!validator.isLength(workAddress, { min: 7, max: 30 })) {
          errorMessage.workAddress =
            'work address must not be less than 7 or above 30 characters';
        }
        if (!validator.isLength(homeAddress, { min: 7, max: 30 })) {
          errorMessage.homeAddress =
            'home address must not be less than 7 or above 30 characters';
        }

        if (!(Object.keys(errorMessage).length === 0)) {
          return res.status(422).json({ status: 'failed', error: errorMessage });
        }
        return next();
      }
    );
